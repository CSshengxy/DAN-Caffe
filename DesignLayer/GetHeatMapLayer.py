...
class GetHeatMapLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        self.half_size = math.floor(PATCH_SIZE / 2)
    ...
    def reshape(self, bottom, top):
        top[0].reshape(bottom[0].shape[0], 1, IMGSIZE, IMGSIZE)

    def forward(self, bottom, top):
        self.landmarks = bottom[0].data
        for i in range(self.landmarks.shape[0]):
            top[0].data[i] = self.getHeatMap(self.landmarks[i])
    ...
    def getHeatMap(self, landmark):
        ...
        imgs = np.zeros((landmark.shape[0], 1, IMGSIZE, IMGSIZE))
        for i in range(landmark.shape[0]):
            imgs[i] = self.getHeatMapHelper(landmark[i])
        img = np.max(imgs, 0)
        return img
